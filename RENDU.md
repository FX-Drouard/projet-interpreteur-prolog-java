...
```
	...
	Dans nos mÈthodes, nous vÈrifions, surtout pour les deux premiers interprËtes, que les programmes vÈrifient bien les conditions de l'ÈnoncÈ. Si ce n'est pas le cas, nous levant l'exception FormatASTNotOK.
	On lËve cette exception dans :
	-interprete0 quand :
		- il y a trop de dÈclarations
		- le but est placÈ avant le fait (ou il n'y a pas de fait)
		- il n'y a pas qu'un seul but
	-interprete1 quand :
		- il y a trop de buts
		- les DeclAssertion ne sont pas que des faits
		- il y a deux faits avec le mÍme symbole de prÈdicat
```